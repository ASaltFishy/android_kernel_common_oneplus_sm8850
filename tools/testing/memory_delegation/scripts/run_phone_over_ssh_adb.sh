...
#   USE_SU=1                       (run test under `su 0 sh -c ...`)
...
#   KILL_STALE_BROKER=1            (kill prior test broker before running)
#
set -euo pipefail
...
ssh ${SSH_OPTS} "${JUMP_HOST}" \
  "REMOTE_TMP=${REMOTE_TMP@Q} PHONE_DIR=${PHONE_DIR@Q} ADB_SERIAL=${ADB_SERIAL@Q} USE_SU=${USE_SU@Q} ADB_BIN=${ADB_BIN@Q} EXTRA_ENV=${EXTRA_ENV@Q} CLEAR_LOGCAT=${CLEAR_LOGCAT@Q} WAIT_SECS=${WAIT_SECS@Q} WAIT_BOOT_COMPLETED=${WAIT_BOOT_COMPLETED@Q} KILL_STALE_BROKER=${KILL_STALE_BROKER@Q} bash -s" <<'EOF' 2>&1 | tee "${OUT_LOG}"
set -euo pipefail
...
if [[ "${KILL_STALE_BROKER}" == "1" ]]; then
  ...
  if [[ "${USE_SU}" == "1" ]]; then
    "${adb_cmd[@]}" shell "su 0 sh -c 'pkill -f \"[s]cudo_shared_arena_test\" || true'" </dev/null || true
  fi
  "${adb_cmd[@]}" shell "pkill -f '[s]cudo_shared_arena_test' || true" </dev/null || true
fi
...
if [[ "${USE_SU}" == "1" ]]; then
  "${adb_cmd[@]}" shell "su 0 sh -c $(printf "%q" "${run_cmd}")" </dev/null
else
...
# echo "== dmesg (tail) =="
# if [[ "${USE_SU}" == "1" ]]; then
#   run_timeout 20 "${adb_cmd[@]}" shell "su 0 -c 'dmesg | tail -n 400'" </dev/null || true
# else
...
if [[ "${USE_SU}" == "1" ]]; then
  run_timeout 30 "${adb_cmd[@]}" shell "su 0 -c 'dmesg'" </dev/null || true
else
